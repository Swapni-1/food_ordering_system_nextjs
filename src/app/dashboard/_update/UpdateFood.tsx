...
const foodSchema = z.object({
    ...
    description : z.string({required_error : "description cannot be empty"}).min(10,"Invalid email address"),
...
export default function UpdateFood({className,food,mutate}){
    ...
    useEffect(() => {
      axios.get("/api/category")
          ...
          data = food.categories.map((f) => f.name === "All" ? {label : f.name,value : f.slug,fixed : true} : {label : f.name,value : f.slug})
    ...
    function handleDialogChange(isOpen : boolean){
        setOpen(isOpen);
        foodForm.setValue("name",food.name);
        foodForm.setValue("description",food.description)
        foodForm.setValue("image",food.foodImg)
        foodForm.setValue("price",`${food.price}`)
        foodForm.setValue("category",option)
        if(!isOpen){
          foodForm.reset();
        }
    }
    ...
    function onSubmit(values: z.infer<typeof foodSchema>) {
        ...
        if(values.name === food.name && values.description === food.description && parseInt(values.price) === food.price && values.image === food.foodImg && lodash.isEqual(values.category,option)){
        ...
        axios.put("/api/food/"+food.food_id,foodData)
        .then(({data}) => {
          console.log(data);
          mutate();
    ...
    return (
        <Dialog open={open} onOpenChange={handleDialogChange}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle className="text-center">Update the food</DialogTitle>
              </DialogHeader>
